  ...
  userForm.addEventListener("submit", async (e) => {
    ...
    const emailData = {
      ...
      message: `
        ...
        <strong>PDF Requested</strong>: ${pdfToOpen || "mykeaportfolio.pdf"}<br/>
        <strong>Submitted At</strong>: ${new Date().toLocaleString()}<br/>
      `,
    };

    try {
      // Send to Google Sheets and Email API with proper error handling
      const results = await Promise.allSettled([
        // Save to Google Sheets with proper headers
      ...
      console.log("Google Sheets:", results[0].status);
      console.log("Email API:", results[1].status);

      // Check if email failed (optional warning)
      if (results[1].status === "rejected") {
        console.warn("Email sending failed, but continuing with download");
      }
