  ...
  userForm2.addEventListener("submit", async (e) => {
    ...
    const emailData = {
      ...
      message: `
        ...
        <strong>Phone</strong>: ${fullPhone}<br/>
        <strong>Submitted At</strong>: ${new Date().toLocaleString()}<br/>
      `,
    };

    console.log("📧 Consultation Form - Email Data:", emailData);

    try {
      // Send to Google Sheets and Email API simultaneously
      console.log("📊 Sending consultation form to Google Sheets...");
      console.log("📧 Sending consultation email...");
      
      const results = await Promise.allSettled([
        // Save to Google Sheets
        fetch("https://script.google.com/macros/s/AKfycbyEK5qTlIvToXe7OdE8vdLyoWMnpHcI-fnry5yqIgCCQQ2KHFpbs1K_0vXeFOvqnipZqQ/exec", {
          method: "POST",
          mode: "no-cors",
          headers: {
        ...
        // Send Email
        fetch("https://mailer-5x4h33dpla-uc.a.run.app/", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          console.log("📧 Consultation Email Response Status:", response.status);
          console.log("📧 Consultation Email Response OK:", response.ok);
          
          const responseText = await response.text();
          console.log("📧 Consultation Email Response Body:", responseText);
          
          if (!response.ok) {
            throw new Error(`Email API returned status ${response.status}: ${responseText}`);
          }
      ...
      console.log("📊 Consultation Google Sheets Result:", results[0]);
      console.log("📧 Consultation Email Result:", results[1]);

      // Check if email failed
      if (results[1].status === "rejected") {
        console.error("❌ Consultation email sending failed:", results[1].reason);
        console.warn("⚠️ Continuing with redirect anyway...");
      } else {
        console.log("✅ Consultation email sent successfully!");
      }

      // Step 2: After successful submission, open new tab
      const newTab = window.open("", "_blank");
